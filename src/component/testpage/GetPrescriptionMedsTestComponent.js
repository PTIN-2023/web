...
export default function GetPrecriptionMedsTestComponent({apiEndpoint}) {
  ...
  })}, [userTokenCookie, prescriptionIdentifier])

  // Define the api call based on the state
  async function apiCall() {
    return fetch(apiEndpoint+"/api/get_prescription_meds", {
    method: 'POST',
    headers: {
  ...
  return(<>
    ...
    <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
      ...
      <div className="mb-2 block">
          <Label
          htmlFor="prescription_identifier"
          value="prescription_identifier"
          />
      </div>
      <TextInput
          ...
          type="text"
          required={true}
          onChange={(e) => setPrescriptionIdentifier(e.target.value)}
      />
      </div>
      <Button type="submit">
      Submit
