...
export function makeServer() {
  return createServer({
    ...
    routes() {
      ...
      this.post('/api/makeorder', (schema, request) => {
        const medicamentos = JSON.parse(request.requestBody).medicamentos;

        medicamentos.forEach((medicamento) => {
          const foundMedicamento = schema.medicamentos.findBy({ medName: medicamento.medName });
          if (foundMedicamento) {
            foundMedicamento.destroy();
          }
        });

        return { message: "Medicamentos eliminados" };
      });

      this.get('/api/medicamentos', (schema) => {
        return schema.medicamentos.all();
      })
