...
export default async function generatePDF(nombrePaciente, nombreMedicamento, tratamiento, notas, renwal, codigo, props) {
  ...
  nombreMedicamento.forEach((medicamento, index) => {
    ...
    page.drawText(getText("name", localeCookie)`: ${medicineName} - ` + getText("ID", localeCookie) + `: ${idMedicamento} - ` + getText("quantity", localeCookie) + `: ${cantidad}`, { x: 50, y: yPosition, size: 15, font });
  });
  page.drawText(getText("renewal", localeCookie) + `: ${renwal}`, { x: 50, y: 590 - (nombreMedicamento.length * 20), size: 15, font });
  page.drawText(getText("duration", localeCookie) + `: ${tratamiento}`, { x: 50, y: 560 - (nombreMedicamento.length * 20), size: 15, font });
  page.drawText(getText("notes", localeCookie) + `:`, { x: 50, y: 530 - (nombreMedicamento.length * 20), size: 15, font });
